...
const MOCK_CONTENT_DETAIL: Record<string, AnyContentData> = {
  c1:  { id: 'c1',  title: '¿Qué es Python?',       type: 'video', url: 'https://www.w3schools.com/html/mov_bbb.mp4', duration: 120, thumbnail: 'https://images.unsplash.com/photo-1526379095098-d400fd0bf935?w=400' },
  c2:  { id: 'c2',  title: 'Instalación y entorno', type: 'text',  body: '<h3>Instalación de Python</h3><p>Visita <strong>python.org</strong>, descarga e instala la versión más reciente.</p><p>Verifica con <code>python --version</code>.</p>' },
  c3:  { id: 'c3',  title: 'Tu primer programa',    type: 'video', url: 'https://www.w3schools.com/html/mov_bbb.mp4', duration: 90 },
  c4:  { id: 'c4',  title: 'Variables y tipos',     type: 'text',  body: '<h3>Variables en Python</h3><p>Se crean al asignarles un valor:</p><pre><code>nombre = "Ana"\nedad = 25</code></pre>' },
  c5:  { id: 'c5',  title: 'Quiz: Fundamentos',     type: 'quiz',
    questions: [
      { id: 'q1', text: '¿Qué símbolo se usa para asignar un valor a una variable?', options: [{ id: 'o1', text: '==' }, { id: 'o2', text: '=' }, { id: 'o3', text: ':=' }, { id: 'o4', text: '->' }], correctOptionId: 'o2' },
      { id: 'q2', text: '¿Cuál es un tipo de dato en Python?',                        options: [{ id: 'o5', text: 'integer' }, { id: 'o6', text: 'int' }, { id: 'o7', text: 'number' }, { id: 'o8', text: 'num' }], correctOptionId: 'o6' },
      { id: 'q3', text: '¿Cómo se imprime "Hola mundo" en Python?',                  options: [{ id: 'o9', text: 'echo(...)' }, { id: 'o10', text: 'console.log(...)' }, { id: 'o11', text: 'print(...)' }, { id: 'o12', text: 'printf(...)' }], correctOptionId: 'o11' },
    ],
  },
...
function getDefaultContent(id: string): AnyContentData {
  return MOCK_CONTENT_DETAIL[id] ?? { id, title: 'Contenido no disponible', type: 'text', body: '<p>Este contenido aún no está disponible.</p>' }
}
...
export function CourseMapPage() {
  ...
  const completedIds   = useMemo(() => progress?.completedContents ?? [], [progress])
  ...
  const globalProgress = totalContents > 0 ? Math.round((completedCount / totalContents) * 100) : 0

  const moduleStatuses   = useMemo(() => computeModuleStatuses(MOCK_COURSE.modules, completedIds), [completedIds])
  const contentStatuses  = useMemo(() => computeContentStatuses(MOCK_COURSE.modules, completedIds), [completedIds])
  ...
  const flatItems = useMemo((): FlatItem[] => {
    ...
    let gi = 0
    for (const [mi, mod] of MOCK_COURSE.modules.entries()) {
          ...
          <div className="lg:sticky lg:top-8 space-y-4">
            ...
            <div className="rounded-2xl overflow-hidden aspect-video bg-primary relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                <span className="text-[10px] font-bold text-tertiary/70 uppercase tracking-widest">Masterclass</span>
                <span className="text-sm font-bold text-tertiary text-center mt-1 leading-snug">{MOCK_COURSE.title}</span>
              </div>
            </div>
...
function ContentCard({ item, onAction }: { item: FlatItem; onAction: () => void }) {
  ...
  if (isCurrent) {
    return (
      <div
        onClick={onAction}
        className="relative overflow-hidden rounded-3xl w-full md:max-w-[210px]
        ...
        <div className="p-3.5">
          ...
          <button
            ...
            className="mt-3 w-full flex items-center justify-center gap-1.5
                       min-h-[36px] px-3 rounded-xl text-white

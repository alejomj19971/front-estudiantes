...
import {
  ...
  Play, GraduationCap, Wrench, Lock, CheckCircle2,
} from 'lucide-react'
...
interface CourseMeta { type: CourseType; instructor: string; module: string }
      ...
      <div className="flex items-center gap-3 p-3 rounded-xl bg-surface-muted">
        ...
        <div>
          ...
          <p className="text-xs text-secondary">
            Te {total - done === 1 ? 'falta' : 'faltan'} {total - done} módulo{total - done !== 1 ? 's' : ''} para tu meta.
          </p>
        </div>
      </div>
    </div>
        ...
        <div className="flex items-center justify-between mt-auto pt-3">
          {status === 'locked' && (
            <>
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full
                               text-[11px] font-semibold bg-mid/15 text-mid">
                Bloqueado
              </span>
              <Lock size={14} className="text-mid" />
            </>
          )}
          {status === 'approved' && (
            <>
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full
                               text-[11px] font-semibold bg-secondary/15 text-secondary">
                Aprobado
              </span>
          ...
          {status === 'progress' && (
            <div className="w-full">
              ...
              <div className="h-1 rounded-full bg-mid/25 overflow-hidden">
                <div
                  className="h-full rounded-full bg-secondary transition-all duration-700"
                  style={{ width: `${course.progress}%` }}
                />
